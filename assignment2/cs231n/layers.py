...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    X_scratch = x.reshape(x.shape[0], -1)
    out = X_scratch.dot(w) + b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # To backword mu(x) = np.sum(x, axis=0)/D
        # mu is (D,)
        sigma = np.var(x, axis=0)
        # sigma is (D,)
        # To backword sigma(x) = np.sum(x_i - mu(x), axis=0)/N
        eta = 1. / np.sqrt(sigma + eps) 
        # eta is (D,)
        # To backword eta(x) = 1/np.sqrt(sigma**2 + eps)
        x_new = (x - mu) * eta
        # x_new is (N, D)
...
#     x_pad[:, :, pad:-pad, pad:-pad] = x
    x_pad = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant') # By document, default set zero.
    out = np.zeros((N, F, H1, W1)) # N, F, H', W'
    # I must admit that 4 nested for loop is some kinds of stupid
    for n in range(N):
        for i in range(H1):
            for j in range(W1):
                for f in range(F):
#                     print(x_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride +  WW].shape)
#                     print(w[f].shape)
...
def conv_backward_naive(dout, cache):
    ...
    db = np.sum(dout, axis = (0, 2, 3)) # Count per f
    stride = conv_param['stride']
    pad = conv_param['pad']
    ...
    for n in range(N):
        for f in range(F):
            for i in range(H1):
                for j in range(W1):
#                     print('i, j', i, j)
#                     print(dx_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride +  WW].shape)
#                     print(dout[n, f, i * stride:i * stride + HH, j * stride:j * stride +  WW].shape)
#                     print(w[f].shape)
#                     out[n, f, i, j] = np.sum(x_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride +  WW] * w[f]) + b[f] 
#                     dx_pad[n, :, i, j] += np.sum(dout[n, f, i * stride:i * stride + HH, j * stride:j * stride +  WW] * w[f])
                    dx_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride +  WW] += dout[n, f, i, j] * w[f]
#                     print(dout[n, f, i * stride:i * stride + HH, j * stride:j * stride +  WW].shape)
#                     print(x_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride +  WW].shape)
#                     print(np.array(dout[n, f, i, j] * x_pad[n, :, i * stride:i * stride + HH, j * stride:j * stride +  WW]).shape)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H1):
                for j in range(W1):
#                     print(np.argmax(dx[n, c, i * stride:i * stride + pool_height, j * stride:j * stride +  pool_width]))
                    idx = np.argmax(x[n, c, i * stride:i * stride + pool_height, j * stride:j * stride +  pool_width])
                    dx_i, dx_j = divmod(idx, pool_width) # A little tricky here since argmax return a single value instead of coordinate
                    dx[n, c, i * stride + dx_i, j * stride + dx_j] += dout[n, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out = out.reshape(N, H, W, C).transpose(0, 2, 3, 1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx = dx.reshape(N, H, W, C).transpose(0, 2, 3, 1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
